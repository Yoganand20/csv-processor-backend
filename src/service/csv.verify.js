...
export default function validateCSVFile(fileName, folderPath) {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (isNaN(currentSNo)) {
        return { valid: false, message: `Row ${i}: Invalid S. No format` };
      }
      ...
      if (!productName.trim()) {
        return { valid: false, message: `Row ${i}: Product Name cannot be empty` };
      }

      // Validate Image URLs
      const urls = imageUrls.split(',').map(url => url.trim()).filter(url => url);
      if (urls.length === 0) {
        return { valid: false, message: `Row ${i}: At least one image URL is required` };
      }
      ...
      for (const url of urls) {
        try {
          ...
          return { valid: false, message: `Row ${i}: Invalid URL format - ${url}` };
        }
      }
    }
    ...
    if (error.code === 'ENOENT') {
      return { valid: false, message: `File not found: ${path.join(folderPath, fileName)}` };
    }
    return { valid: false, message: `Error processing file: ${error.message}` };
  }
}
